...
def show_telemetry(
    ...
    json_output: bool = typer.Option(
        False,
        "--json-output",
        help="If set, print telemetry records as JSON instead of table format.",
    ),
) -> None:
...
def fetch_api(
    ...
    """
    Fetch arbitrary API endpoint and print the response.
    """
...
@devices_cli.command("upload-file")
def upload_file(
    api_url: str = typer.Argument(...),
    api_key: str = typer.Option(..., "--api-key"),
    file_path: str = typer.Option(..., "--file"),
    domain_id: str = typer.Option(..., "--domain-id"),
    folder_id: int = typer.Option(1, "--folder-id"),
    workspace_id: int = typer.Option(1, "--workspace-id")
):
    files = {
    ...
    url = f"{api_url}/file/workspaces/{workspace_id}/folders/{folder_id}/files/upload"
